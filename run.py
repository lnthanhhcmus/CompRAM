...
class Runner(object):
    ...
    def add_model(self, model, score_func):
        ...
        if model_name.lower() == 'compram_distmult':
            model = CompRAMDistMult(self.edge_index, self.edge_type, params=self.p)
        elif model_name.lower() == 'compram_complex':
            model = CompRAMComplEx(self.edge_index, self.edge_type, params=self.p)
        else:
    ...
    def run_epoch(self, epoch, val_mrr=0):
        ...
        for step, batch in enumerate(train_iter):
            ...
            loss.backward()
            #torch.nn.utils.clip_grad_norm_(self.model.parameters(), self.p.clip)
            self.optimizer.step()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-model', dest='model', default='compram', help='Model Name')
    parser.add_argument('-score_func', dest='score_func', default='complex', help='Score Function for Link prediction')
    parser.add_argument('-opn', dest='opn', default='sub', help='Message Passing Operation to be used')
