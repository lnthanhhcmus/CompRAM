...
class MessagePassing(torch.nn.Module):
    ...
    def __init__(self, aggr='add'):
        ...
        self.update_args = inspect.getargspec(self.update)[0][3:]
    ...
    def propagate(self, aggr, edge_index, **kwargs):
        ...
        for arg in self.message_args:
            if arg[1:3] == '_i':  # If arguments ends with _i then include indic
                k = arg[:1] + arg[3:]
                tmp = kwargs[k]  # Take the front part of the variable | Mostly it will be 'x',
                size = tmp.size(0)
                message_args.append(tmp[edge_index[0]])  # Lookup for head entities in edges
            elif arg[1:3] == '_j':
                k = arg[:1] + arg[3:]
                tmp = kwargs[k]  # tmp = kwargs['x']
                size = tmp.size(0)
        ...
        out_real, out_imaginary = self.message(*message_args)
        if self.p.att is None:
            out_real = scatter_(aggr, out_real, edge_index[0], dim_size=size)  # Aggregated neighbors for each vertex
            out_imaginary = scatter_(aggr, out_imaginary, edge_index[0], dim_size=size)
        out_real, out_imaginary = self.update(out_real, out_imaginary, *update_args)
